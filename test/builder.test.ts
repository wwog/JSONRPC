...
describe("JsonRpcBuilder", () => {
  // 使用 CounterIdGenerator 来简化测试，使 ID 可预测
  beforeEach(() => {
  ...
  describe("createSuccessResponse", () => {
    it("应创建标准成功响应对象", () => {
      ...
      const response = JsonRpcBuilder.createSuccessResponse(result, id);
    ...
    it("应支持不同类型的结果数据", () => {
      // 字符串结果
      expect(JsonRpcBuilder.createSuccessResponse("string result", 1)).toEqual({
        jsonrpc: "2.0",
        id: 1,
      ...
      // 数值结果
      expect(JsonRpcBuilder.createSuccessResponse(42, 2)).toEqual({
        jsonrpc: "2.0",
        id: 2,
        result: 42,
      });
      ...
      // null 结果
      expect(JsonRpcBuilder.createSuccessResponse(null, 3)).toEqual({
        jsonrpc: "2.0",
        id: 3,
        result: null,
      });
